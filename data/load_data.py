...
def load_filtered_data(pkl_scannet_file, refers, relation, use_sem, ins_sem_mapping, add_pad=True):
    ...
    refer_datas = []
    for rf, rel in zip(refers, relation):
        refer_data = pd.read_csv(rf)
        if rel != "":
            relation_data = pd.read_csv(rel)
            refer_data['relations'] = relation_data['relations']
        else:
            refer_data['relations'] = None
        refer_datas.append(refer_data)
    referit_data = pd.concat(refer_datas)
    ...
    referit_data = referit_data[['tokens', 'instance_type', 'scan_id', 'is_train',
                                 'dataset', 'target_id', 'utterance', 'stimulus_id', 'relations']]
...
def load_scanrefer_data(pkl_scannet_file, train_file, val_file, use_sem, ins_sem_mapping, add_pad=True):
    with open('./data/scannet/scannetv2_val.txt', 'r') as f:
        val_scan_ids = f.readlines()
    val_scan_ids = [x.strip() for x in val_scan_ids]
    ...
    for k, scan_list in all_scans.items():
        for scan in scan_list:
            if scan.scan_id in val_scan_ids:
                object_num.append(len(scan.three_d_objects))
            if use_sem:
                class_labels.update([ins_sem_mapping[o.instance_label] for o in scan.three_d_objects])
            else:
                class_labels.update([o.instance_label for o in scan.three_d_objects])

    threshold = np.percentile(object_num, 95)
    # print(threshold)
