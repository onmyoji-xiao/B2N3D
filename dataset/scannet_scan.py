...
class ScannetScan(object):
    ...
    def load_point_clouds_of_all_objects(self, exclude_instances=None):
        ...
        self.point2object = np.zeros(self.pc.shape[0], dtype=np.int32) - 1
        all_objects = []
        for object_info in scan_aggregation['segGroups']:
            ...
            self.point2object[object_pc] = object_id

        self.three_d_objects = all_objects
        return check

    def load_all_objects_by_maskcluster(self,seg_path,val=False):
        npz = np.load(os.path.join(seg_path, self.scan_id,
                         "output/object/scannet/object_dict.npy"),
            allow_pickle=True)
        group_dict = npz.item()
        ...
        self.target_boxes = dict()
        self.target_points = dict()
        scan_aggregation_suffix = '.aggregation.json'
        aggregation_file = osp.join(self.dataset.top_scan_dir, self.scan_id, self.scan_id + scan_aggregation_suffix)
        with open(aggregation_file) as fin:
            scan_aggregation = json.load(fin)
        ...
        ins2id = {}
        pc_ins_ids = np.zeros(self.pc.shape[0], dtype=np.int32)
        for object_info in scan_aggregation['segGroups']:
            object_instance_label = object_info['label']
            if object_instance_label not in ins2id:
                ins2id[object_instance_label] = len(ins2id)
            ...
            for s in segments:
                pc_loc.extend(segment_indices_dict[s])
            if self.sample:
                pc_loc = [pc_map[pi] for pi in pc_loc if pi in self.selected_pis]
            pc_ins_ids[pc_loc] = ins2id[object_instance_label]
            obj_pc = self.pc[pc_loc]
            xmin = min(obj_pc[:, 0])
            ymin = min(obj_pc[:, 1])
            zmin = min(obj_pc[:, 2])
            xmax = max(obj_pc[:, 0])
            ymax = max(obj_pc[:, 1])
            zmax = max(obj_pc[:, 2])
            self.target_boxes[object_id] = [np.array(
                [(xmin + xmax) / 2, (ymin + ymax) / 2, (zmin + zmax) / 2, xmax - xmin, ymax - ymin, zmax - zmin]),
                object_instance_label]
            self.target_points[object_id] = pc_loc
        all_objects = []
        id2ins = {v: k for k, v in ins2id.items()}
        for inst_id in group_dict:
            object = group_dict[inst_id]
            object_pc = object['point_ids']
            if len(object_pc) < 50:
                continue
            if self.sample:
                object_pc = [pc_map[pi] for pi in object_pc if pi in self.selected_pis]
            counts = np.bincount(pc_ins_ids[object_pc])
            instance_id = np.argmax(counts)
            if val == False and counts[instance_id] < len(object_pc) * 0.5:
                continue
            instance_label = id2ins[instance_id]
            all_objects.append(ThreeDObject(inst_id, object_pc, instance_label))
        self.three_d_objects = all_objects
