...
class Nr3dDataset(Dataset):
    ...
    def get_reference_data(self, index):
        ...
        if relations is not None:
            target_cls = target.instance_label
            if not isinstance(relations, str) or relations == '':
                relations = target_cls
            mem = []
            for pair in relations.split(','):
                if '-' in pair:
                    ...
                    if len(entities) != 2:
                        continue
                    first = entities[0] if entities[0] in self.ins_sem_mapping else None
                    second = entities[1] if entities[1] in self.ins_sem_mapping else None
                    if first is not None and second is not None:
                        relation_pairs.append([first, second])
                        mem.append(first)
                        mem.append(second)
                elif pair in self.ins_sem_mapping:
                    relation_pairs.append([pair, pair])
                    mem.append(pair)
            if target_cls not in mem and target_cls in self.ins_sem_mapping:
                relation_pairs.append([target_cls, target_cls])
                for mm in mem:
                    relation_pairs.append([target_cls, mm])

        return scan, target, tokens, scan_id, relation_pairs
    ...
    def __getitem__(self, index):
        ...
        scan, target, tokens, scan_id, relation_pairs = self.get_reference_data(index)
        ...
        with h5py.File(h5path, "r") as f:
            for d_id in f[scan_id]:
                ...
                # selected_ = np.random.choice(h5_feats.shape[0], size=min(3, h5_feats.shape[0]), replace=False)
                # object_dict[int(d_id)] = np.mean(h5_feats[selected_], axis=0)
        clip_feats = []
        ...
        re_labels = np.zeros((self.max_context_size, self.max_context_size))
        object_sems = res['class_labels']
        for pair in relation_pairs:
            cls_a = self.ins_sem_mapping[pair[0]]
            cls_b = self.ins_sem_mapping[pair[1]]
            if cls_a not in self.class_to_idx:
                cls_a = plural_to_singular(cls_a)
            if cls_b not in self.class_to_idx:
                cls_b = plural_to_singular(cls_b)
            if cls_a not in self.class_to_idx or cls_b not in self.class_to_idx:
                continue
            a_ci = self.class_to_idx[cls_a]
            b_ci = self.class_to_idx[cls_b]
            a_inds = np.where(object_sems == a_ci)[0]
            b_inds = np.where(object_sems == b_ci)[0]
            if len(a_inds) > 0 and len(b_inds) > 0:
                a_repeated = np.repeat(a_inds, len(b_inds))
                b_tiled = np.tile(b_inds, len(a_inds))
                re_labels[a_repeated, b_tiled] = 1
                re_labels[b_tiled, a_repeated] = 1
        o_labels = (np.sum(re_labels, axis=-1) > 0).astype(float)
        o_labels[target_pos] = 1
        re_labels[np.arange(0, self.max_context_size), np.arange(0, self.max_context_size)] = 0

        res['o_labels'] = o_labels
        res['re_labels'] = re_labels
