...
class CrossAttn(nn.Module):
    def __init__(self,
                 ...
                 dropout):
        super().__init__()
        layer = CrossFFN(n_head=head_num, d_model=inner_dim, d_hidden=hidden_dim,
                         dropout=dropout)
        self.crossattn = nn.ModuleList([copy.deepcopy(layer) for _ in range(enc_layer)])

    def forward(self, features, text):
        for layer in self.crossattn:
            features = layer(features, text)
        return features
